...
pub fn sub() {
    ...
    let op_desc = unsafe {
        ...
        let c = c.as_raw();
        Descriptor::new(
            |ptr| infiniop! { infini_op::bindings::infiniopCreateSubDescriptor(handle, ptr, c, a, b) },
            infini_op::bindings::infiniopDestroySubDescriptor,
        )
    };

    let mut workspace_size: usize = 0;
    infiniop! { infini_op::bindings::infiniopGetSubWorkspaceSize(op_desc.as_raw(), &mut workspace_size) };
    let mut workspace: Vec<u8> = vec![0; workspace_size];

    infiniop! { infini_op::bindings::infiniopSub(
        op_desc.as_raw(),
