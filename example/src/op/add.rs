...
pub fn add() {
    ...
    let op_desc = unsafe {
        ...
        Descriptor::new(
            |ptr| {
                infiniop! { infini_op::bindings::infiniopCreateAddDescriptor(handle, ptr, c, a, b) }
            },
            infini_op::bindings::infiniopDestroyAddDescriptor,
        )
    };

    let mut workspace_size: usize = 0;
    infiniop! { infini_op::bindings::infiniopGetAddWorkspaceSize(op_desc.as_raw(), &mut workspace_size) };
    let mut workspace: Vec<u8> = vec![0; workspace_size];

    infiniop! { infini_op::bindings::infiniopAdd(
        op_desc.as_raw(),
